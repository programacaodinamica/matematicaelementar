...
# ╔═╡ 7625f148-3c85-11ee-2b65-99e3de4ff220
using Plots

# ╔═╡ 27fac99b-9605-4c87-81e5-73a86156c0e8
md"""
# Função Modular
"""

# ╔═╡ 970f80cc-90dd-4ba8-ba76-f08c9c1a44cc
f(x) = if (x ≥ 0) x else -x end

# ╔═╡ a55513e7-1424-441a-8376-4d6ccb18d877
x = -4:0.2:4

# ╔═╡ 0c3795f5-4a24-4861-a1c1-9b7692867f56
plot(x, f.(x), framestyle=:origin)

# ╔═╡ 20ace533-ff82-4298-bc81-5805a99b66cb
g1(x) = -(x^2)/2

# ╔═╡ 4e3336bc-f4d1-4bc6-85b5-24d0c0ce3bd5
plot(x, [g1.(x), f.(g1.(x))], framestyle=:origin, aspect_ratio = :equal )

# ╔═╡ 5a47e91a-6d1c-4e13-81eb-3d821e79960d
begin
x2 = -2:0.2:10
g2(x) = x^2 -8x + 12
g3(x) = -x^2 +8x - 12

plot(x2, [g2.(x2), g3.(x2), f.(g3.(x2))], framestyle=:origin)
end

# ╔═╡ fb13d875-fb76-43a0-9f05-dab84563156c
begin
	x3 = -2π:0.02:2π
	h(x) = sin(x)
	plot(x3, [h.(x3), f.(h.(x3))], framestyle=:origin)
end
